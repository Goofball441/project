  ...
  function visualization(states) {
    ...
    let extent =d3.extent(states , function(d){return d.income});
    
    //Define colour scale
      selectedColor = d3['interpolateRdBu'];
    ...
    let circlesGroup = chartGroup.selectAll('circle').data(states).enter().append('circle')
      ...
      .attr('r','17').attr('fill',function(d){return colorScale(d.income)}).attr('opacity','0.85')
    ...
    const toolTip = d3.tip().attr('class','d3-tip').offset([80,-65])
      .html(function(d){return(`<strong>${d.state}</strong><br>Poverty: ${d.poverty}%<br>Obesity: ${d.obesity}%<br>Income: $${d.income}`)})
    ...
    //create legend
    const linearGradient = chartGroup.append("defs")
      ...
      const colorAxis = d3.axisBottom(d3.scaleLinear()
                          ...
                          .range([0,400]))
                          .ticks(5).tickSize(-20);
      chartGroup.append('g').call(colorAxis)
